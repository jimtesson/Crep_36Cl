function best_age = Find_age(Const_cosmo, Param_cosmo, Param_site, Sf, dataset, erosion, t_guess,flag)
...
f = @(t_expo,erosion) inv_misfit(Const_cosmo,Param_cosmo,Param_site,Sf,dataset,dataset(3,:),erosion,t_expo,flag); 
...
elseif(strcmp(flag.search,'nmsmax')==1)
    ...
    [best_age,Obj3,N3] = nmsmax(@(x) -f(x,erosion),guess,[precx inf inf 0 0]);
