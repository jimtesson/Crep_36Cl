function P = Protons(h,Rct,s,consts,nuclide)
...
Ine=ones(1,length(E));  % vecteur ligne unité de taille E
...
%  debut de l'ancienne boucle
    ...
    if nuclide == 3
        P.P3p = (trapz(E,phiPtot.*(Inrc*(consts.OpxHe3T + consts.SipxHe3T./2)),2).*consts.Natoms3.*1e-27.*3.1536e7)';    
    elseif nuclide == 10
        P.P10p = (trapz(E,phiPtot.*(Inrc*(consts.O16pxBe10 + consts.SipxBe10./2)),2).*consts.Natoms10.*1e-27.*3.1536e7)';
    elseif nuclide == 14    
        P.P14p = (trapz(E,phiPtot.*(Inrc*(consts.O16pxC14 + consts.SipxC14./2)),2).*consts.Natoms14.*1e-27.*3.1536e7)';
    elseif nuclide == 26
        P.P26p = (trapz(E,phiPtot.*(Inrc*consts.SipxAl26),2).*consts.Natoms26.*1e-27.*3.1536e7)'; 
    else
        P.pflux = (trapz(E(clipindex:end),phiPtot(:,clipindex:end),2))';
    end
