...
Beta = sqrt(1-(Emu./(Emu + E)).^2); % Particle speed relative to light _ length of E
c = 3.0e8; % speed of light, in m/s
p = sqrt((E.^2 + 2.*E.*Emu)); % in MeV/c   _ length of E
...
% for a = 1:length(Rc)
    %Negative Muons
    v11nmin = w111nmin + w112nmin.*Rc + w113nmin./(1 + exp((Rc - w114nmin)./w115nmin));  %length of Rc
    ...
    mflux.total = trapz(E(clipindex:end),Phimu(:,clipindex:end),2);
    % Differential fluxes for negative and positive muons
    mflux.neg = phimun;
    mflux.pos = phimup;
    % Integral fluxes for positive and negative muons
    mflux.nint = trapz(E(clipindex:end),phimun(:,clipindex:end),2);
    mflux.pint = trapz(E(clipindex:end),phimup(:,clipindex:end),2);
